		...
		<div class="wrapper">
				...
				<div class="head-content">
					<table cellpadding="0" cellspacing="0" width="100%" border="0">
						...
						@foreach($headers['meta'] as $name => $value)
							...
							<tr>
							@endif
								<td style="width: 1%; white-space: nowrap;"><span style="color:#808080;">{{ $name }}</span>:</td>
								<td style="padding-left: 5px;">{{ ucwords($value) }}</td>
							@if ($metaCtr % 2 == 1)
							</tr>
							@endif
							<?php $metaCtr++; ?>
				...
				<table width="100%" class="table">
					...
					@if ($showTotalColumns != [] && $ctr > 1)
						<tr class="bg-black f-white">
							@if ($showNumColumn || $grandTotalSkip > 1)
								<td colspan="{{ $grandTotalSkip }}"><b>Total Estimasi</b></td> {{-- For Number --}}
							@endif
							<?php $dataFound = false; ?>
							@foreach ($columns as $colName => $colData)
								@if (array_key_exists($colName, $showTotalColumns))
			...
			<div class="sign">
				<div class="person person-1">
					<div class="head">
						<p>Mengajukan,</p>
						<p>{{ $query->get()[0]->jobtitle }}</p>
					</div>
					<div class="name">
						<p>({{ $query->get()[0]->first_name }} {{ $query->get()[0]->last_name }})</p>
					</div>
				</div>
				<div class="person person-2">
					<div class="head">
						<p>Mengetahui,</p>
						<p>Kabiro Keuangan & Sarana Prasarana</p>
					</div>
					<div class="name">
						<p>(Elza Rendrawati, S.E.)</p>
					</div>
				</div>
			</div>
		</div>
